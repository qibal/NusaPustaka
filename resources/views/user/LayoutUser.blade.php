                ...
                <div class="flex items-center gap-4">
                    <div class="sm:flex sm:gap-4">
                        @guest
                            ...
                            @if (Route::has('loginMahasiswa'))
                                <a href="{{ route('loginMahasiswa') }}"
                                    class="rounded-md bg-gray-100 px-5 py-2.5 text-sm font-medium text-teal-600">Login</a>
                            @endif
                        @else
                            <a class="rounded-md bg-gray-100 px-5 py-2.5 text-sm font-medium text-teal-600"
                                href="{{ route('logout-mahasiswa') }}"
                                onclick="event.preventDefault();
                             document.getElementById('logout-form').submit();">
                                {{ __('logout') }}
                            </a>

                            <form id="logout-form" action="{{ route('logout-mahasiswa') }}" method="POST" class="d-none">
                                @csrf
                            </form>
                        @endguest
